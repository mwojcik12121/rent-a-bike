...
function getAllResults()
{
    ...
    if($query = mysqli_prepare($db, "SELECT * FROM reklamacje WHERE id_reklamacji = ?"))
    {
        mysqli_stmt_bind_param($query, "i", $id);
        mysqli_stmt_execute($query);
        mysqli_stmt_bind_result($query, $rid, $userid, $state, $date, $desc, $rentid);
        $row = mysqli_stmt_fetch($query);
        
        echo "<br><p style= \"font-size: 24px; color: white\">Numer reklamacji: ".$rid."</p>";
        echo "<br><p style= \"font-size: 24px; color: white\">Data złożenia: ".$date."</p>";
        echo "<br><p style= \"font-size: 24px; color: white\">Stan: ".$state."</p>";
        echo "<br><p style= \"font-size: 24px; color: white\">Identyfikator użytkownika: ".$userid."</p>";
        
        if($query2 = mysqli_prepare($db, "SELECT * FROM klient WHERE id_klienta = ?"))
        {
            mysqli_bind_param($query2, "", $userid);
            mysqli_stmt_execute($query2);
            mysqli_stmt_bind_result($query2, $usrid, $mail, $tel, $money, $pwd, $user);
            $row = mysqli_stmt_fetch($query2);
            
            echo "<br><p style= \"font-size: 24px; color: white\">Klient: ".$user." (".$usrid.")</p>";
        }
        
        echo "<br><p style= \"font-size: 24px; color: white\">Opis: ". $desc ."</p>";
    }
    mysqli_close($db);
}
...
<head>
  <title>Reklamacje - Rent-A-Bike</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
    ...
    <div
        ...
        <div style="background-color: rgba(255,255,255,0.3); width: 1700px; height: 800px; position: absolute; top: 53%; left: 50%; transform: translate(-50%, -50%); padding: 30px">
                <div style="width: 400px; height: 740px; background-color: rgba(255,255,255,0.3)">
                    <button type="button" style="width: 90%; height: 20%; margin: 19px; background-color: deeppink; border-color: deeppink" onclick="location.href='../complaints'" class="btn btn-primary btn-lg btn-block"><b>Reklamacje</b></button>
                    <button type="button" style="width: 90%; height: 20%; margin: 19px; background-color: aqua; border-color: aqua" onclick="location.href='../users'" class="btn btn-primary btn-lg btn-block"><b>Baza wypożyczających</b></button>
            ...
            <div class="col-lg-6 my-4" style="display: inline-block; text-align: center; padding: 70px; width: 1200px; height: 740px; background-color: rgba(255,255,255,0.3); position: absolute; top: 50%; left: 61%; transform: translate(-50%, -50%)">
                ...
                <h1 style="font-size: 48px; color: white"><b>Reklamacje</b></h1>
                
                <div div style="margin: 20px; margin-top: 40px; padding: 5px; background-color: rgba(255,255,255,0.3); height: 500px">
                    <div style="margin: 5px; padding: 20px; text-align: left; height: 480px">
                        <?php getAllResults();?>
                    </div>
                </div>
