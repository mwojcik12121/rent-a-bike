...
function getLoggedUser()
{
    ...
    if($query = mysqli_prepare($db, "SELECT * FROM pracownicy WHERE id_pracownika = ?"))
    {
        mysqli_stmt_bind_param($query, "s", $_SESSION["userId"]);
        mysqli_stmt_execute($query);
        mysqli_stmt_bind_result($query, $rid, $mail, $tel, $pwd, $usr);
        $row = mysqli_stmt_fetch($query);
            
        echo "<br><br><br><h1 style=\"font-size: 48px; color: white\"><b>" . $usr . "</b></h1><br><br>";
        echo "<h1 style=\"font-size: 28px; color: white\">Adres e-mail: " . $mail . "</h1><br><br>";
        echo "<h1 style=\"font-size: 28px; color: white\">Tel: " . $tel . "</h1><br>";
    }
    mysqli_close($db);
}
...
<head>
  <title>Mój profil - Rent-A-Bike</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
    ...
    <div
        ...
        <div style="background-color: rgba(255,255,255,0.3); width: 1700px; height: 800px; position: absolute; top: 53%; left: 50%; transform: translate(-50%, -50%); padding: 30px">
                <div style="width: 400px; height: 740px; background-color: rgba(255,255,255,0.3)">
                    ...
                    <button type="button" onclick="location.href='../map'" style="width: 90%; height: 20%; margin: 19px; background-color: aqua; border-color: aqua" class="btn btn-primary btn-lg btn-block"><b>Wyświetl mapę</b></button>
                </div>
            </div>
            ...
            <div class="col-lg-6 my-4" style="display: inline-block; text-align: center; padding: 70px; width: 1200px; height: 740px; background-color: rgba(255,255,255,0.3); position: absolute; top: 50%; left: 61%; transform: translate(-50%, -50%)">
                <img src="profile.png" class="img-fluid" alt="<user>" style="width: 20%; height: auto"/>
                <?php getLoggedUser(); ?>
            </div>
        </div>
